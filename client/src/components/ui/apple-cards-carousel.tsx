                ...
                {open && (
                    <div className="fixed inset-0 z-50 h-screen overflow-auto">
                        <motion.div
                            ...
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
                            className="fixed inset-0 h-full w-full bg-[#191919]/90 backdrop-blur-sm"
                        />
                        <motion.div
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                        ...
                        >
                            ...
                            <motion.p
                                ...
                                className="text-base font-medium text-[#f45025]"
                            >
                                {card.category}
                            </motion.p>
                            <motion.p
                                layoutId={layout ? `title-${card.title}` : undefined}
                                className="mt-4 text-2xl font-bold text-[#191919] md:text-5xl uppercase"
                            >
                                {card.title}
                            </motion.p>
                            <div className="py-10 text-[#191919]">{card.content}</div>
                        </motion.div>
                    </div>
                )}
            </AnimatePresence>
            ...
            >
                ...
                <div className="relative z-40 p-8">
                    <motion.p
                        ...
                        className="text-left font-sans text-sm font-medium text-[#f45025] md:text-base uppercase tracking-wider"
                    >
                        {card.category}
                    </motion.p>
                    <motion.p
                        layoutId={layout ? `title-${card.title}` : undefined}
                        className="mt-2 max-w-xs text-left font-sans text-xl font-bold [text-wrap:balance] text-white md:text-3xl uppercase"
                    >
                        {card.title}
                    </motion.p>
                </div>
